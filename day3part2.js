 ...
 * @returns {(string|number)} The direction and distance from the definition
 */
 ...
 * @returns {number[]} The new ending X,Y position
 */
 ...
 * @returns {Set<number>} The points in the line
 */
...
const findDistanceToPoint = (definitions, targetX, targetY) => {
    ...
    definitions.split(',').some(definition => {
        ...
        if (points.has(targetPoint)) {
            ...
            let [_, lineDistance] = parseDefinition(definition);
...
/**
 * Calculate the minimum number of steps to the intersection between the two wires
 ...
 * @returns {number} The minimum number of steps to the intersection
 */
const calculateMinmumNumberOfSteps = (wire1Definition, wire2Definition) => {
    ...
    const intersections = Array.from(wire1Points).filter(value => wire2Points.has(value));
